...
exports.categories = async function(req, res) {
  ...
  const foundCats = await categoryCollection.find()
...
exports.items = async function(req, res) {
  ...
  let page = req.query.page
  const searchQuery = req.query.search
  const foundProduct = await productCollection.find()
      .skip((resPerPage * page) - resPerPage)
      .limit(resPerPage)
      .toArray()
   // count how many categories founded 
...
// dont forget to add error message to the deleted file
  ...
  let page = req.query.page
  const searchQuery = req.query.search
  const foundCats = await customerCollection.find()
      .skip((resPerPage * page) - resPerPage)
      .limit(resPerPage)
      .toArray()
   // count how many categories founded 
  ...
  // end pagination
      ...
      res.render('backend/customers', {
        ...
        users : await customerCollection.aggregate([
          {$lookup : {
            from : "products",
  ...
  exports.pages = async function(req, res){
    ...
    let page = req.query.page
    const searchQuery = req.query.search
    const foundCats = await pagesCollection.find()
        .skip((resPerPage * page) - resPerPage)
        .limit(resPerPage)
        .toArray()
     // count how many categories founded 
  ...
  exports.downloads = async function(req, res) {
    ...
    let page = req.query.page
    const searchQuery = req.query.search
    const foundCats = await customerCollection.aggregate([
      {$lookup : {
        from : "products",
        localField : "productId",
        foreignField : "_id",
  ...
  ]).skip((resPerPage * page) - resPerPage).limit(resPerPage).toArray()
     // count how many categories founded 
