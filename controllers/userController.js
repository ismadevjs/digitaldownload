		...
		if(o) {
			...
			paypal.payment.execute(paymentId, execute_payment_json, async function (error, payment) {
			 ...
			 // find and delete repétition
				 ...
				 await usersCollection.updateOne({_id: ObjectID(req.session.user._id)}, {$push : {
					productId : o._id,
...
exports.downloads = async function(req, res) {		
			res.render('frontend/downloads', {
				...
				pages : await pageCollection.find().toArray(),
				products :  await productCollection.find().toArray(),
				users : await usersCollection.aggregate([
		...
		if (err) {
			res.redirect('/404')
			 ...
			 if (req.originalUrl == "/browse/"+founded._id) {
			  ...
			  const foundCats = await productCollection.aggregate([
				{$lookup : {
					from : "categories",
